...
and female bill lengths and body masses, across all three penguin species.

```{r}
mod_penguins |> 
    filter(!is.na(sex)) |> 
    select(species, sex, bill_length_mm, body_mass_g) |> 
    # Combine bill_length_mm and body_mass_g into two long columns ("metric" and
    # "value"), so we can use them as faceting variables.
...
these two traits, but we haven't applied any sort of rigorous test. In this
lesson, we'll add statistical analyses to our R skill set.
